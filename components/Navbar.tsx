...
const Navbar: React.FC = () => {
  ...
  // Logic Click Outside untuk menutup menu
  useEffect(() => {
  ...
  return (
    <nav className="sticky top-0 z-40 border-b border-slate-800/60 bg-slate-950/80 backdrop-blur-md">
      ...
      {mobileMenuOpen && (
        <div 
            ref={menuRef}
            className="md:hidden absolute right-4 top-16 w-56 origin-top-right rounded-xl bg-slate-900 border border-slate-800 shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-200"
        >
            <div className="px-2 py-2 space-y-1">
                <Link 
                    href="/dashboard/laporan" 
                    className="block px-4 py-2.5 rounded-lg text-sm font-medium text-slate-200 hover:bg-slate-800 hover:text-white transition"
                ...
                <button
                    ...
                    className="block w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium text-red-400 hover:bg-slate-800 hover:text-red-300 transition"
                >
                    🚪 Logout
                </button>
            </div>
