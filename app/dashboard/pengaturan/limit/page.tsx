...
export default function LimitSettingsPage() {
    ...
    try {
      ...
      setForm({
        daily_limit: data.daily_limit
          ? formatRupiah(String(data.daily_limit))
          : "",
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const payload = {
      daily_limit: parseInt(cleanRupiah(form.daily_limit)) || 0,
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 space-y-8">
      ...
      <div className="text-center space-y-2">
        <div className="w-12 h-12 bg-amber-500/10 text-amber-500 rounded-2xl flex items-center justify-center mx-auto text-2xl mb-4">
          🚨
        </div>
        <h2 className="text-xl font-bold text-white">
          Atur Pesan Limit Harian
        </h2>
        <p className="text-sm text-slate-400">
          Bot Telegram akan mengirim alert jika melebihi batas limit.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    daily_limit: formatRupiah(e.target.value),
                  })
                }
            ...
            <p className="text-[10px] text-slate-500 mt-2">
              Isi 0 untuk mematikan fitur limit.
            </p>
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-2">
              Pesan Peringatan di Bot Telegram
            </label>

            <textarea
              value={form.alert_message}
              onChange={(e) =>
                setForm({ ...form, alert_message: e.target.value })
              }
              rows={3}
            ...
            <p className="text-[10px] text-slate-500 mt-2">
              Pesan ini dikirim otomatis saat limit terlampaui.
            </p>
          </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-4 flex gap-3 items-start">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 leading-relaxed">
            Gunakan pesan yang tegas agar lebih sadar pengeluaran harian.
          </p>
        </div>
      </div>
    </div>
